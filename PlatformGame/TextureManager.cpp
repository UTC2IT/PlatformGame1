...
bool TextureManager::Load(std::string id, std::string filename)
{
	...
	SDL_Texture* texture = SDL_CreateTextureFromSurface(Game::getInstance()->getRenderer(), surface); //tham số đầu tiên là trình Render, tham số thứ 2 là cấu trúc SDL_Surface chữa dữ liệu pixel được sử dụng để lấp đầy texture
	if (texture == nullptr)
	{
...
bool TextureManager::ParseTextures(std::string source)
{
	...
	if (xml.Error())
	{
		std::cout << "Faile to load" << source << std::endl;
		return false;
	}
...
{
	SDL_Rect srcRect = { tileSize * frame, tileSize * row, tileSize, tileSize };//source điểm đầu của ảnh
