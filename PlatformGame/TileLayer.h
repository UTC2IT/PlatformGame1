...
public:
	...
	virtual void Update();
	void RgidMap();
	TileMap GetTileMap() { return mTileMap; }//để dùng Biến m_TileMap ở các class khác(ví dụ như xử lý va chạm)
	int getTilesize() { return mTilesize; }
	int getRowCount() { return mRowCount; }
	int getColCount() { return mColCount; }
	std::vector<SDL_Rect> GetCollisionRect() { return mCollisionRect; }
private:
